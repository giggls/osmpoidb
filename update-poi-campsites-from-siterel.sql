...
FROM
  (
    SELECT member_id, osm_type, site_tags
    FROM osm_poi_camp_siterel
    WHERE member_tags->'tourism'='camp_site'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'amenity' = 'post_box'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'amenity' = 'drinking_water'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND ( r.member_tags ? 'shop' AND r.member_tags -> 'shop' != 'laundry')
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND ((r.member_tags->'amenity' = 'washing_machine') OR (r.member_tags->'shop' = 'laundry'))
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'firepit'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'amenity' = 'toilets'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND ((r.member_tags->'amenity' = 'shower') OR ((r.member_tags->'amenity' = 'toilets') AND (r.member_tags ? 'shower') AND (r.member_tags->'shower' != 'no')))
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'playground'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'swimming_pool'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'golf_course'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'miniature_golf'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'leisure' = 'sauna'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'amenity' = 'fast_food'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    ...
    FROM
      ...
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags -> 'amenity' = 'restaurant'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
UPDATE                                 
  osm_poi_campsites cs
SET
  sport = ARRAY(SELECT DISTINCT UNNEST(cs.sport || sr.sport))
FROM
  (
    SELECT
      s.member_id, s.osm_type, array_agg(r.member_tags->'sport') as sport
    FROM
      osm_poi_camp_siterel s
      INNER JOIN osm_poi_camp_siterel r ON s.site_id = r.site_id
      AND s.member_tags -> 'tourism' = 'camp_site'
      AND r.member_tags ? 'sport' group by s.member_id,s.osm_type
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    SELECT member_id, osm_type, site_id
    FROM osm_poi_camp_siterel
    WHERE member_tags->'tourism'='camp_site'
  ) sr
WHERE
...
FROM
  (
    ...
    WHERE site_id IN
     ...
     AND member_tags -> 'tourism' = 'camp_site'
  ) sr
WHERE
  cs.osm_id = sr.member_id AND cs.osm_type = imposm_type2str(sr.osm_type);
...
FROM
  (
    SELECT s.member_id, bool_and(st_within(r.geom, s.geom)) AS within
    FROM osm_poi_camp_siterel_with_geom s
    INNER JOIN osm_poi_camp_siterel_with_geom r ON s.site_id = r.site_id
    AND s.member_tags -> 'tourism' = 'camp_site'
    AND ((r.member_tags -> 'tourism' != 'camp_site') OR NOT (r.member_tags ? 'tourism'))
    GROUP BY s.member_id
  ) sr
WHERE
  sr.within = true AND cs.osm_id = sr.member_id;
